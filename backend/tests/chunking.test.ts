...
function makeDoc(sentenceCount: number): string {
  const parts: string[] = [];
...
describe('splitSentences()', () => {
  it('splits flowing plain text into sentences', () => {
    const sents = splitSentences(CLINICAL_SENTS.slice(0, 3).join(' '));
  ...
  it('preserves section headers as atomic units', () => {
    const sents = splitSentences(STRUCTURED_DOC);
  ...
  it('handles single sentence without error', () => {
    const sents = splitSentences(CLINICAL_SENTS[0]);
...
describe('TextChunker — short document (fits in one chunk)', () => {
  const chunker = new TextChunker({ maxTokensPerChunk: 500, overlapTokens: 20, countTokens });
...
describe('TextChunker — large document (acceptance criteria a / b / c)', () => {
  ...
  const MAX_TOKENS = 50;
  const OVERLAP = 5;
  const chunker = new TextChunker({ maxTokensPerChunk: MAX_TOKENS, overlapTokens: OVERLAP, countTokens });

  const largeDoc = makeDoc(30);
  let chunks: ReturnType<TextChunker['chunk']>;
  ...
  it('(c) chunks cover ≥ 95% of source sentences', () => {
    const sourceSents = splitSentences(largeDoc);
    const allText = chunks.map((c) => c.text).join(' ');
    let covered = 0;
    for (const s of sourceSents) {
      const probe = s.slice(0, 20).trim();
      if (probe.length > 5 && allText.includes(probe)) covered++;
    }
...
describe('TextChunker — structured document with section headers', () => {
  const chunker = new TextChunker({ maxTokensPerChunk: 25, overlapTokens: 3, countTokens });
...
describe('TextChunker — chunkDocuments() multi-doc', () => {
  const chunker = new TextChunker({ maxTokensPerChunk: 40, overlapTokens: 4, countTokens });
...
describe('HierarchicalSummarizer — short document', () => {
  const s = new HierarchicalSummarizer({
