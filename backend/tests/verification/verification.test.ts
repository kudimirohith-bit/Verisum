import { VerificationPipeline } from '../../src/verification/index';
import { benchmarkDataset } from './benchmarkData';
...
describe('Verification & Hallucination Detection Benchmark Suite (0.7)', () => {
  ...
  it('runs 20-pair synthetic benchmark and reports precision/recall metrics', async () => {
    ...
    for (const pair of benchmarkDataset) {
      ...
      console.log(
        `| ${pair.id} | ${pair.docType} | ${pair.isFaithful ? 'Faithful' : 'Hallucinated'} | ${isFlagged ? 'Flagged' : 'Clean'} | ${result.flaggedClaims.length} | ${result.consistencyScore} | ${matchStatus} |`,
      );
    }
