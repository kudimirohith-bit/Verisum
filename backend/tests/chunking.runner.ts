...
import { TextChunker } from '../src/chunking/TextChunker';
import { HierarchicalSummarizer } from '../src/chunking/HierarchicalSummarizer';
import { splitSentences } from '../src/chunking/splitter';
...
test('splitSentences: single sentence without error', () => {
  const s = splitSentences(SENTS[0]);
  assert.ok(s.length >= 1);
});

// ── TextChunker — constructor validation ──────────────────────────────────────
test('TextChunker: throws on maxTokensPerChunk = 0', () => {
  assert.throws(() => new TextChunker({ maxTokensPerChunk: 0, overlapTokens: 0, countTokens }));
});

test('TextChunker: throws if overlapTokens >= maxTokensPerChunk', () => {
  assert.throws(() => new TextChunker({ maxTokensPerChunk: 10, overlapTokens: 10, countTokens }));
});

test('TextChunker: throws on negative overlapTokens', () => {
  assert.throws(() => new TextChunker({ maxTokensPerChunk: 10, overlapTokens: -1, countTokens }));
});
...
const MAX = 50;
const OVL = 5;
const chunker = new TextChunker({ maxTokensPerChunk: MAX, overlapTokens: OVL, countTokens });
...
test('structured doc: sectionName detected', () => {
  ...
  const c = new TextChunker({ maxTokensPerChunk: 15, overlapTokens: 2, countTokens });
...
test('chunkDocuments: each doc starts at chunkIndex 0', () => {
  ...
  for (const id of ['X', 'Y']) {
    assert.strictEqual(
      ch.find((x) => x.docId === id)?.chunkIndex,
      0,
      `${id} first chunk not at index 0`,
    );
  }
});
...
async function runAsyncTests() {
  const hs = new HierarchicalSummarizer({
  ...
  try {
    ...
    console.error(' ✗ HierarchicalSummarizer: short doc -', (e as Error).message);
    failed++;
  }
  ...
  try {
    ...
    console.error(' ✗ HierarchicalSummarizer: long doc -', (e as Error).message);
    failed++;
  }
  ...
  try {
    ...
    const l0 = r.chunkSummaries.filter((c) => c.level === 0);
    l0.forEach(({ sentenceRange: [a, b] }) =>
      assert.ok(b > a, `invalid sentenceRange [${a}, ${b}]`),
    );
    console.log(' ✓ HierarchicalSummarizer: sentenceRange [start, end] valid');
    passed++;
  } catch (e: unknown) {
    console.error(' ✗ HierarchicalSummarizer: sentenceRange -', (e as Error).message);
    failed++;
  }
  ...
  try {
    ...
    console.error(' ✗ HierarchicalSummarizer: multi-doc -', (e as Error).message);
    failed++;
  }
  ...
  try {
    ...
    console.error(' ✗ HierarchicalSummarizer: three-doc job -', (e as Error).message);
    failed++;
  }
}
