...
const REGEX_RULES: { pattern: string; tag: string; flags?: string }[] = [
...
// ── Regex-based de-identification pass ────────────────────────────────────────
export interface DeidResult {
  deidentifiedText: string;
  phiMatchCount: number;
  matchedTags: string[];
}

export function regexDeid(text: string): DeidResult {
  let result = text;
  let phiMatchCount = 0;
...
// ── NER pass via model-service (presidio/spaCy) ───────────────────────────────
interface NerEntity {
  text: string;
  label: string;
  start: number;
 ...
 */
async function nerDeid(text: string): Promise<NerResponse | null> {
  try {
    const resp = await axios.post<NerResponse>(
      `${MODEL_SERVICE_URL}/deid/ner`,
      { text },
      { timeout: NER_TIMEOUT_MS },
    );
    return resp.data;
  } catch {
 ...
 */
export async function deidentify(rawText: string): Promise<DeidResult> {
  // Step 1: regex pass
