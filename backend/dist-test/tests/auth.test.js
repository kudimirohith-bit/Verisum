...
// ── Helpers ────────────────────────────────────────────────────────────────────
async function createUser(email, password, role = 'researcher') {
    const hash = await bcryptjs_1.default.hash(password, 1); // low cost for speed in tests
    return User_1.UserModel.create({ email, role, passwordHash: hash });
}
// ── POST /auth/register ────────────────────────────────────────────────────────
...
describe('GET /auth/me', () => {
    ...
    it('returns 401 with no token', async () => {
        const res = await (0, supertest_1.default)(app_1.app).get('/auth/me');
        expect(res.status).toBe(401);
        expect(res.body.error).toBe('Unauthorized');
    });
    it('returns 401 with an expired token', async () => {
        ...
        const expiredToken = jsonwebtoken_1.default.sign({ sub: user._id.toString(), email: user.email, role: user.role, type: 'access' }, process.env.JWT_SECRET || 'dev-secret-change-me', { expiresIn: -1 });
        const res = await (0, supertest_1.default)(app_1.app)
            .get('/auth/me')
...
describe('RBAC — requireRole middleware', () => {
    it('allows an admin to reach /auth/admin/users', async () => {
        ...
        const token = (0, tokens_1.signAccessToken)({ sub: admin._id.toString(), email: admin.email, role: admin.role });
        const res = await (0, supertest_1.default)(app_1.app)
    ...
    it('returns 403 when a researcher calls an admin-only endpoint', async () => {
        ...
        const token = (0, tokens_1.signAccessToken)({ sub: researcher._id.toString(), email: researcher.email, role: researcher.role });
        const res = await (0, supertest_1.default)(app_1.app)
            .get('/auth/admin/users')
            .set('Authorization', `Bearer ${token}`);
    ...
    it('returns 403 when a clinician calls an admin-only endpoint', async () => {
        ...
        const token = (0, tokens_1.signAccessToken)({ sub: clinician._id.toString(), email: clinician.email, role: clinician.role });
        const res = await (0, supertest_1.default)(app_1.app)
            .get('/auth/admin/users')
            .set('Authorization', `Bearer ${token}`);
        expect(res.status).toBe(403);
...
describe('POST /auth/refresh', () => {
    it('returns 401 when no refresh token cookie is sent', async () => {
        const res = await (0, supertest_1.default)(app_1.app).post('/auth/refresh');
        expect(res.status).toBe(401);
    });
    it('issues a new access token with a valid refresh token cookie', async () => {
        ...
        const refreshToken = (0, tokens_1.signRefreshToken)(user._id.toString());
        const res = await (0, supertest_1.default)(app_1.app)
            .post('/auth/refresh')
    ...
    it('returns 401 with an expired refresh token', async () => {
        ...
        const expiredRefresh = jsonwebtoken_1.default.sign({ sub: user._id.toString(), type: 'refresh' }, process.env.JWT_SECRET || 'dev-secret-change-me', { expiresIn: -1 });
        const res = await (0, supertest_1.default)(app_1.app)
            .post('/auth/refresh')
