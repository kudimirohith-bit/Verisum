...
const TextChunker_js_1 = require("../src/chunking/TextChunker.js");
const HierarchicalSummarizer_js_1 = require("../src/chunking/HierarchicalSummarizer.js");
const splitter_js_1 = require("../src/chunking/splitter.js");
...
describe('0.5 — Text Chunking & Hierarchical Summarizer Unit Tests', () => {
    describe('splitSentences', () => {
        it('splits 3 sentences', () => {
            const s = (0, splitter_js_1.splitSentences)(SENTS.slice(0, 3).join(' '));
            expect(s.length).toBe(3);
        });
        it('empty string returns []', () => {
            expect((0, splitter_js_1.splitSentences)('')).toEqual([]);
        });
        it('preserves Dr. abbreviation without splitting incorrectly', () => {
            const s = (0, splitter_js_1.splitSentences)('Dr. Smith reviewed the chart. The patient was stable.');
            expect(s.length).toBeLessThanOrEqual(3);
            const drSent = s.find((x) => x.includes('Dr'));
            expect(drSent?.includes('Smith')).toBe(true);
        });
        it('preserves section headers', () => {
            const s = (0, splitter_js_1.splitSentences)('Assessment:\nNSTEMI suspected. Plan:\nAdmit to CCU.');
    ...
    describe('TextChunker Constructor Validation', () => {
        it('throws on maxTokensPerChunk = 0', () => {
            expect(() => new TextChunker_js_1.TextChunker({ maxTokensPerChunk: 0, overlapTokens: 0, countTokens })).toThrow();
        });
        it('throws if overlapTokens >= maxTokensPerChunk', () => {
            expect(() => new TextChunker_js_1.TextChunker({ maxTokensPerChunk: 10, overlapTokens: 10, countTokens })).toThrow();
        });
        it('throws on negative overlapTokens', () => {
            expect(() => new TextChunker_js_1.TextChunker({ maxTokensPerChunk: 10, overlapTokens: -1, countTokens })).toThrow();
        });
    });
    describe('TextChunker Chunking Logic', () => {
        it('short doc returns single chunk', () => {
            const c = new TextChunker_js_1.TextChunker({ maxTokensPerChunk: 500, overlapTokens: 20, countTokens });
        ...
        it('(a) no chunk exceeds maxTokensPerChunk and (b) ends at sentence boundary', () => {
            const MAX = 50;
            const chunker = new TextChunker_js_1.TextChunker({ maxTokensPerChunk: MAX, overlapTokens: 5, countTokens });
        ...
        it('(c) chunks cover >= 95% of source sentences', () => {
            const chunker = new TextChunker_js_1.TextChunker({ maxTokensPerChunk: 50, overlapTokens: 5, countTokens });
            const largeDoc = makeDoc(30);
            const chunks = chunker.chunk(largeDoc, 'large');
            const sents = (0, splitter_js_1.splitSentences)(largeDoc);
        ...
        it('chunkDocuments tags docId and sets chunkIndex 0 per document', () => {
            const c = new TextChunker_js_1.TextChunker({ maxTokensPerChunk: 40, overlapTokens: 4, countTokens });
    ...
    describe('HierarchicalSummarizer', () => {
        let hs;
        beforeEach(() => {
            hs = new HierarchicalSummarizer_js_1.HierarchicalSummarizer({
